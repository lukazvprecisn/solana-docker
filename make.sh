...
cd "${WORKING_DIR}"
docker build --build-arg "ANCHOR_VERSION"="${ANCHOR_VERSION}" --build-arg "CRITERION_VERSION"="${CRITERION_VERSION}" --build-arg "PLATFORM_TOOLS_VERSION"="${PLATFORM_TOOLS_VERSION}" --build-arg "SOLANA_VERSION=${SOLANA_VERSION}" -t "solana-build:${SOLANA_VERSION}" .
