...
if [ -d "${WORKING_DIR}/.cache/solana-platform-tools-${PLATFORM_TOOLS_VERSION}-linux" ]; then
	echo "Dir solana-platform-tools-${PLATFORM_TOOLS_VERSION}-linux is already existed. Skip downloading."
else
	curl -fSL "https://github.com/anza-xyz/platform-tools/releases/download/v${PLATFORM_TOOLS_VERSION}/platform-tools-linux-x86_64.tar.bz2" -o "${WORKING_DIR}/.cache/solana-platform-tools-${PLATFORM_TOOLS_VERSION}-linux.tar.bz2"
	HASH=$(sha256sum "${WORKING_DIR}/.cache/solana-platform-tools-${PLATFORM_TOOLS_VERSION}-linux.tar.bz2" | gawk '{ print $1 }')
	if [ "${HASH}" = "${PLATFORM_TOOLS_HASH}" ]; then
		echo "Extracting Solana Platform Tools v${PLATFORM_TOOLS_VERSION}"
		mkdir -p "${WORKING_DIR}/.cache/solana-platform-tools-${PLATFORM_TOOLS_VERSION}-linux"
		tar -xvf "${WORKING_DIR}/.cache/solana-platform-tools-${PLATFORM_TOOLS_VERSION}-linux.tar.bz2" -C "${WORKING_DIR}/.cache/solana-platform-tools-${PLATFORM_TOOLS_VERSION}-linux"
	else
...
cd "${WORKING_DIR}"
docker build --file Dockerfile.v3 --build-arg "RUST_IMAGE_TAG"="${RUST_IMAGE_TAG}" --build-arg "ANCHOR_VERSION"="${ANCHOR_VERSION}" --build-arg "CRITERION_VERSION"="${CRITERION_VERSION}" --build-arg "PLATFORM_TOOLS_VERSION"="${PLATFORM_TOOLS_VERSION}" --build-arg "SOLANA_VERSION=${SOLANA_VERSION}" -t "solana-build:${SOLANA_VERSION}-a${ANCHOR_VERSION}" .
